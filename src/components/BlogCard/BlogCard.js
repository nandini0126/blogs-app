...
function BlogCard({id, title, preImg, description, content, author, date, categories }) {
    return (
        <Link className="blog-card" to={`/blog/${id}`}>
            ...
            <img src={preImg} className="pre-img"></img>
            <h2>{title}</h2>
            <p className="desc">{description}..</p>

            <div className="author">
                <img src={author.authImg} className="auth-img" />
                <span className="auth-name">{author.name}</span>
            </div>

            <div className="comment">
                <img src="https://cdn-icons-png.flaticon.com/128/134/134718.png" className="com-img" />
